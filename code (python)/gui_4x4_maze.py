...
def set_screen_qtable(screen , ns = 16 , na = 4 , wr = 34 , wh = 34):
...
def set_screen_board(screen ,maze , episode, a = 4 , h = 50 ,h2 = 34 , ns =16 , na = 4):
    head_width = 75
...
def update_screen_qtabe(screen , q , h = 34 , ns =16 , na = 4):
    font = pygame.font.SysFont('Arial',15)
    ...
    for j in range(0,na):
        for i in range(0,ns//2):
            val = q[i][j] 
            label = font.render(str("{:.1f}".format(val)), 2, WHITE)
...
class maze:
    ...
    def healthy(self,agent):
        '''checks if agent is in a safe house(not wall)'''
...
class Qlearn:
    def __init__(self, num_states, num_actions, lr =0.8 , d_factor=0.5):
    ...
    def train(self , maze0 , n_episodes = 10, score_limit = -0.5 ):
        pygame.init()
